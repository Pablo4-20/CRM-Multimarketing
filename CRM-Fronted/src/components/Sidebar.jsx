    ...
    <div className={`${isCollapsed ? 'w-20' : 'w-[260px]'} bg-white border-r border-slate-200 transition-all duration-300 flex flex-col overflow-hidden shrink-0 z-10`}>
      ...
      <ul className="flex-1 px-4 flex flex-col gap-2 m-0 list-none overflow-y-auto overflow-x-hidden">
        ...
        <li 
          ...
          className={`px-4 py-3 rounded-lg cursor-pointer flex items-center gap-3 font-medium transition-colors ${activeTab === 'mis_clientes' ? 'bg-teal-50 text-teal-600 font-semibold' : 'text-slate-500 hover:bg-slate-50'}`}
        >
        ...
        {isAdmin && (
          <>
            ...
            <li onClick={() => setActiveTab('usuarios')} className={`px-4 py-3 rounded-lg cursor-pointer flex items-center gap-3 font-medium transition-colors ${activeTab === 'usuarios' ? 'bg-blue-50 text-blue-600 font-semibold' : 'text-slate-500 hover:bg-slate-50'}`}>
              <FiUserCheck className="text-xl shrink-0" />
              {!isCollapsed && <span className="truncate">Gestión Usuarios</span>}
            </li>
            
            <li onClick={() => setActiveTab('asignaciones')} className={`px-4 py-3 rounded-lg cursor-pointer flex items-center gap-3 font-medium transition-colors ${activeTab === 'asignaciones' ? 'bg-slate-800 text-white font-semibold shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}>
              <FiCheckSquare className="text-xl shrink-0" />
              {!isCollapsed && <span className="truncate">Asignaciones</span>}
            </li>

            <li onClick={() => setActiveTab('campanas')} className={`px-4 py-3 rounded-lg cursor-pointer flex items-center gap-3 font-medium transition-colors ${activeTab === 'campanas' ? 'bg-blue-50 text-blue-600 font-semibold' : 'text-slate-500 hover:bg-slate-50'}`}>
            ...
            <li onClick={() => setActiveTab('estados')} className={`px-4 py-3 rounded-lg cursor-pointer flex items-center gap-3 font-medium transition-colors ${activeTab === 'estados' ? 'bg-amber-50 text-amber-600 font-semibold' : 'text-slate-500 hover:bg-slate-50'}`}>
              <FiTag className="text-xl shrink-0" />
              {!isCollapsed && <span className="truncate">Estados</span>}
            </li>

            <li onClick={() => setActiveTab('clientes')} className={`px-4 py-3 rounded-lg cursor-pointer flex items-center gap-3 font-medium transition-colors ${activeTab === 'clientes' ? 'bg-indigo-50 text-indigo-600 font-semibold' : 'text-slate-500 hover:bg-slate-50'}`}>
      ...
      <div className={`p-5 border-t border-slate-200 flex items-center gap-3 ${isCollapsed ? 'justify-center flex-col' : 'justify-start'}`}>
        ...
        <button onClick={onLogout} title="Cerrar Sesión" className="text-red-500 hover:text-white hover:bg-red-500 p-2 rounded-lg transition-colors flex items-center justify-center">
